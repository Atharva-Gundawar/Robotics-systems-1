...
class Object_detect():
    ...
    def color_detect(self, img, color):
        ...
        for _ in a:
            ...
            mask = cv2.inRange(hsv, self.HSV[color][0], self.HSV[color][1])
       ...
       # read camera
        ...
        if init_num < 20:
            ...
            detect.set_cut_params(
                (detect.sum_x1)/20.0 -24,
                (detect.sum_y1)/20.0 +24,
                (detect.sum_x2)/20.0 +24,
                (detect.sum_y2)/20.0 -24,
            )
        ...
        if count < 2:
            ...
            detect_result = detect.color_detect(frame,color)
...
if __name__ == "__main__":
    ...
    while True:
        ...
        for color, (lower, upper) in HSV.items():
            ...
            if contours:
                ...
                # If the detected color region has a larger area, update the result
                if area > max_area:
                    max_area = area
        ...
        if detected_color:
            ...
            cv2.putText(frame, f"Color: {detected_color}", (10, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
